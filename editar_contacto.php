...
$results = $database->execute_query("SELECT * FROM contato WHERE id = :id", $params);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    //check if there is another contato with the same number

    $results = $database->execute_query("SELECT id FROM contato WHERE id !=:id AND telefone = :telefone", $params);
    ...
    if ($results->affected_rows != 0) {
        ...
        $results = $database->execute_non_query("UPDATE contato SET nome = :nome, telefone = :telefone, updated_at = NOW() 
        WHERE id = :id", $params);
        header('Location: index.php');
    }
}
